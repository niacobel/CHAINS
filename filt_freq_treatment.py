...
########################################################################################################################################################
##                                                       Frequency filters results treatment                                                          ##
##                                                                                                                                                    ##
##                     This script compiles the results of the frequency filters variation, then identifies the most interesting combos               ##
...
def check_abspath(path:str,context:str,type="either"):
    ...
    if not os.path.exists(path):
      raise FF_Error (msg + "ERROR: %s does not seem to exist." % path)
    elif type == "file":
      if not os.path.isfile(path):
        raise FF_Error (msg + "ERROR: %s is not a file" % path)
    elif type == "directory":
      if not os.path.isdir(path):
        raise FF_Error (msg + "ERROR: %s is not a directory" % path)
    elif type == "either":
      if not os.path.isdir(path) and not os.path.isfile(path):
        raise FF_Error (msg + "ERROR: %s is neither a file nor a directory" % path)
...
parser = argparse.ArgumentParser(add_help=False, description="This script compiles the results of the frequency filters variation, then identifies the most interesting combos and copies their associated pulse and PCP files from the GLOBALSCRATCH to the target directory.")

required = parser.add_argument_group('Required arguments')
required.add_argument("-r","--results_dir", type=str, help="Path to the directory containing the results of the frequency filters variation.", required=True)
...
def main(): 
  ...
  try:
    ...
    print("")
    print("FREQUENCY FILTERS RESULTS TREATMENT BEGINS NOW".center(columns))
    print("")
    print("".center(columns,"*"))
    ...
    results_dir = args.results_dir           # Path to the directory containing the results of the frequency filters variation
  ...
  try:
    ...
    # Only keep the xxx_windX'-type directories

    dir_list = []

    for dirname in dir_list_all:
 
      # Define the 'xxx_windX' regex and apply it to the dirname

      pattern = re.compile(r"^\d+_(?P<suffix>wind\d+)$")
      ...
      if matching_dir is not None:
        ...
        alpha_pattern = re.compile(r'^\s+alpha\s+=\s+(?P<alpha>\d+\.\d+d[+-]\d+)')
        ...
        for line in param_content:
          ...
          if alpha_pattern.match(line): 
            alpha = float((alpha_pattern.match(line).group('alpha')).replace('d','e'))
        ...
        for pcp_dir in pcp_dirs:
          ...
          if line_data is not None:
            ...
            raise FF_Error ("ERROR: Unable to get the values from the file %s" % pcp_file_path) 
    ...
    if dir_list == []:
      raise FF_Error ("ERROR: Can't find any 'xxx_windXXX' directory in %s" % results_dir)
  ...
  with open(os.path.join(out_dir,'filt_freq_comp_results.csv'), 'w', newline='', encoding='utf-8') as csvfile:

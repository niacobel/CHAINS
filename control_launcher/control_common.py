...
def atomic_unit_conversion(value:float,system:str,type:str) -> float:
    ...
    if system not in ['si','au']:
      raise ControlError ("ERROR: The unit system of the value (%s) is neither 'si' nor 'au'" % system)
    elif type not in conv_factors_low.keys():
      raise ControlError ("ERROR: The type of the value you want to convert (%s) is currently not supported. Supported types include: %s" % (type, ', '.join(unit for unit in conv_factors.keys())))
